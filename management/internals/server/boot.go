...
func (s *BaseServer) IDPHandler() http.Handler {
	...
	dexHandler := cors.AllowAll().Handler(embeddedIdP.Handler())
	return mfagate.New(dexHandler, embeddedIdP.DexStorage(), s.Store())
}

// DownloadsHandler returns the HTTP handler for client install downloads, or nil if disabled.
func (s *BaseServer) DownloadsHandler() http.Handler {
	return downloads.Handler(cors.AllowAll())
}
