	...
	if s, ok := servers.mgmtSrv.GetContainer(mgmtServer.ContainerKeyBaseServer); ok {
		if baseServer, ok := s.(*mgmtServer.BaseServer); ok {
			baseServer.AfterInit(func(s *mgmtServer.BaseServer) {
				...
				s.SetHandlerFunc(createCombinedHandler(grpcSrv, s.APIHandler(), s.IDPHandler(), s.DownloadsHandler(), servers.relaySrv, servers.metricsServer.Meter, cfg))
...
func createCombinedHandler(grpcServer *grpc.Server, httpHandler http.Handler, idpHandler http.Handler, downloadsHandler http.Handler, relaySrv *relayServer.Server, meter metric.Meter, cfg *CombinedConfig) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case downloadsHandler != nil && strings.HasPrefix(r.URL.Path, "/downloads/"):
